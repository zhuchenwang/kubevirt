...
type ConsoleHandler struct {
	...
	vsockStopChans       map[types.UID](chan struct{})
	serialLock           *sync.Mutex
	vncLock              *sync.Mutex
	vsockLock            *sync.Mutex
...
func NewConsoleHandler(podIsolationDetector isolation.PodIsolationDetector, vmiInformer cache.SharedIndexInformer) *ConsoleHandler {
	return &ConsoleHandler{
		...
		vsockStopChans:       make(map[types.UID]chan struct{}),
		serialLock:           &sync.Mutex{},
		vncLock:              &sync.Mutex{},
		vsockLock:            &sync.Mutex{},
		usbredirLock:         &sync.Mutex{},
...
func (t *ConsoleHandler) VSOCKHandler(request *restful.Request, response *restful.Response) {
	...
	uid := vmi.GetUID()
	stopChn := newStopChan(uid, t.vsockLock, t.vsockStopChans)
	defer deleteStopChan(uid, stopChn, t.vsockLock, t.vsockStopChans)
